...
async def serve_portal():
    """Serve the monitoring portal HTML."""
    portal_path = os.path.join(os.path.dirname(__file__), "portal.html")
    if os.path.exists(portal_path):
        async with aiofiles.open(portal_path, "r") as f:
            return await f.read()
...
async def dashboard(user=Depends(verify_token)):
    """Summary stats for the portal dashboard."""
    conn = get_db()
...
async def get_students(
    ...
    """Paginated list of all students with current status."""
    conn = get_db()
...
async def upload_excel(file: UploadFile = File(...), user=Depends(verify_token)):
    """Upload/replace the students Excel file."""
...
async def download_excel(user=Depends(verify_token)):
    """Download the current (updated) Excel file."""
